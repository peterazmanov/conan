...
from conans.util.files import sha256sum, md5sum, sha512sum
...
class CompleteFlowTest(unittest.TestCase):
    ...
    def upload_only_tgz_if_needed_test(self):
        ...
        for server_hash_alg in (md5sum, sha256sum, sha512sum):
            test_server = TestServer(hash_algorithm=server_hash_alg)
            self.servers = {"default": test_server}
            self.client = TestClient(servers=self.servers, users={"default": [("lasote", "mypass")]})
